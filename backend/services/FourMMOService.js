...
class FourMMOService {
  constructor(apiKey) {
    this.apiKey = apiKey;
    this.baseUrl = "https://4mmo.net";
  }
  ...
  async shortenUrl(url, options = {}) {
    try {
      ...
      console.error("4MMO shorten error:", error.message);
      return {
        success: false,
  ...
  async testConnection() {
    try {
      ...
      console.error("4MMO test connection error:", error.message);
      return {
        success: false,
        message:
          error.response?.data?.message ||
          error.message ||
          "Không thể kết nối đến 4MMO API",
      };
    }
  }
}

export default FourMMOService;
