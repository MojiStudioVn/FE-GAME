...
/**
 * LinkNgon.io API Service
 * Documentation: linkngon.io API
 *
 * Features:
 * - Shorten URL
 * - Supports JSON and TEXT format
 * - Supports custom alias and sub_link
 */

class LinkNgonIoService {
  constructor(apiKey) {
    this.apiKey = apiKey;
    this.baseUrl = "https://linkngon.io";
  }
   ...
   * @param {string} options.sub_link - Backup link (link dự phòng)
   * @returns {Promise<object>} Shortened URL response
   */
  async shortenUrl(url, options = {}) {
    try {
      const { alias, sub_link } = options;
      ...
      if (sub_link) params.append("sub_link", sub_link);
      ...
      console.error("LinkNgon.io shorten error:", error.message);
      return {
        success: false,
  ...
  async testConnection() {
    try {
      ...
      console.error("LinkNgon.io test connection error:", error.message);
      return {
        success: false,
        message:
          error.response?.data?.message ||
          error.message ||
          "Không thể kết nối đến LinkNgon.io API",
      };
    }
  }
}

export default LinkNgonIoService;
