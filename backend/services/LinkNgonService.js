...
/**
 * LinkNgon.com API Service
 * Documentation: linkngon.com API
 *
 * Features:
 * - Shorten URL
...
class LinkNgonService {
  constructor(apiKey) {
    this.apiKey = apiKey;
    this.baseUrl = "https://linkngon.com";
  }
  ...
  async shortenUrl(url, options = {}) {
    try {
      ...
      console.error("LinkNgon shorten error:", error.message);
      return {
        success: false,
  ...
  async testConnection() {
    try {
      ...
      console.error("LinkNgon test connection error:", error.message);
      return {
        success: false,
        message:
          error.response?.data?.message ||
          error.message ||
          "Không thể kết nối đến LinkNgon API",
      };
    }
  }
}

export default LinkNgonService;
