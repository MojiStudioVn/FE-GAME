...
const DEFAULT_GLOW_COLOR = '184, 115, 51';
...
const GlobalSpotlight: React.FC<any> = ({ gridRef, disableAnimations = false, enabled = true, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, glowColor = DEFAULT_GLOW_COLOR }) => {
  ...
  useEffect(() => {
    ...
    const handleMouseLeave = () => {
      ...
      gridRef.current?.querySelectorAll('.magic-bento-card').forEach((card: any) => card.style.setProperty('--glow-intensity', '0'));
...
const BentoCardGrid: React.FC<any> = ({ children, gridRef }) => (
      ...
      <BentoCardGrid gridRef={gridRef}>
        {cardsToRender.map((card, index) => {
          ...
          const cardProps: any = {
            ...
            style: { backgroundColor: card.color ?? '#060010', ['--glow-color' as any]: glowColor }
          };
          ...
          return (
            <div key={index} {...cardProps} ref={el => {
              if (!el) return;
